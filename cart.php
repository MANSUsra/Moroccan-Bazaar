...
if (isset($_POST['add_to_cart'])) {
    if (isset($_SESSION['user_id'])) {
        ...
        $upsertQuery = "INSERT INTO user_cart (user_id, product_id, quantity)
                        VALUES (?, ?, ?)
                        ON DUPLICATE KEY UPDATE quantity = quantity + ?";
        $stmt = mysqli_prepare($mysqli, $upsertQuery);
        if (!$stmt) {
            die("Error in SQL query: " . mysqli_error($mysqli));
        }
        mysqli_stmt_bind_param($stmt, "iiii", $userId, $productId, $quantity, $quantity);
        if (mysqli_stmt_execute($stmt)) {
            // Product added or quantity updated successfully
        } else {
            die("Error in SQL query: " . mysqli_error($mysqli));
        }
    }
...
<body>
        ...
        <div>
            <ul id="navbar">
                ...
                <div id="grp2">
                    ...
                    ?>
                    <li><a class="active" href="cart.php" id="lg-bag"><i class="fa-solid fa-cart-shopping"></i></a></li>
                </div>
            </ul>
        </div>
        <div id="mobile"> 
    ...
    <section id="cartSection">
        ...
        <?php
            ...
            if (isset($_SESSION["user_id"])) {
                $userId = $_SESSION["user_id"];
                $query = "SELECT c.product_id, p.name, p.price, c.quantity, p.image
                        FROM user_cart c
                        JOIN cart_items p ON c.product_id = p.product_id
                ...
                if ($result && mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        <tr>
                            <td>
                            <a href='cart.php?action=remove&product_id=" . $productId ."'>
                            <button>
